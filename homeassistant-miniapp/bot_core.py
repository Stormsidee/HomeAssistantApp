    ...
    def handle_house_commands(self, chat_id: int, text: str, area_name: str) -> None:
        ...
        if text_lower == 'назад':
            ...
            if not self.send_message(chat_id, "Неизвестная команда", "house", area_name):
                logger.error("Не удалось отправить сообщение")def control_single_device(self, chat_id: int, text: str, area_name: str) -> None:
...
Состояния:"""
    ...
    def show_all_houses_status(self, chat_id: int) -> None:
        ...
        for area in areas_list:
            devices = areas.get(area, [])on_count = len([d for d in devices if d['state'] == 'on'])
